...
export default function SignUpPage() {
  ...
  const handleEmailSignUp = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/v1/seltra/auth/sign-up`,
        { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name, email, password }) }
      )
      const data = await res.json()
      if (!res.ok || !data.success) { setError(data.message || 'Sign up failed'); setLoading(false); return }
      localStorage.setItem('seltra_session', JSON.stringify(data.session))
      // New users: onboarding not done yet
      setAuthCookies(data.session.token, false)
  ...
  return (
    <div className="auth-card animate-fade-in" style={{ padding: '36px 32px' }}>
      ...
      {mode === 'options' ? (
        <>
          <h1 style={{ textAlign: 'center', fontSize: 20, fontWeight: 700, marginBottom: 6, letterSpacing: '-0.02em' }}>Create your account</h1>
          <p style={{ textAlign: 'center', fontSize: 13, color: 'var(--text-3)', marginBottom: 28 }}>Your store will be live in minutes.</p>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            <p style={{ textAlign: 'center', marginTop: 8, fontSize: 13, color: 'var(--text-3)' }}>
              ...
              <Link href="/sign-in" style={{ color: 'var(--green)', textDecoration: 'none', fontWeight: 500 }}>Sign in</Link>
            </p>
          </div>
        </>
      ) : (
        <form onSubmit={handleEmailSignUp} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <h2 style={{ fontSize: 18, fontWeight: 600, marginBottom: 4 }}>Create with email</h2>
          <p style={{ fontSize: 13, color: 'var(--text-3)', marginBottom: 4 }}>Just a few details to get started.</p>
          <input type="text" className="input-base" placeholder="Full name" value={name} onChange={e => setName(e.target.value)} required autoFocus />
          <input type="email" className="input-base" placeholder="you@store.com" value={email} onChange={e => setEmail(e.target.value)} required />
          <input type="password" className="input-base" placeholder="Password (min 8 chars)" value={password} onChange={e => setPassword(e.target.value)} required minLength={8} />
          {error && (
