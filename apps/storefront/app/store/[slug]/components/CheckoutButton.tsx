...
// export function CheckoutButton({ cart, store, currency, totalAmount, accentColor, accentFg }: CheckoutButtonProps) {
...
//   const generateRef = () => `seltra_${store.slug}_${Date.now()}_${Math.random().toString(36).slice(2, 7)}`
...
//           // Verify on backend
//           try {
//             await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/v1/seltra/orders/verify`, {
//               method: 'POST',
...
//             })
//           } catch {
//             // Non-blocking — order captured on webhook too
//           }
//           setStep('idle')
//           setLoading(false)
...
//   // Step 1: idle — show Checkout button
//   if (step === 'idle') {
//     return (
//       <button
...
//           style={{
//             background: 'transparent', border: `1px solid ${accentColor}44`, color: '#e5e7eb',
//             padding: '10px 14px', borderRadius: 4, fontSize: 13, outline: 'none', width: '100%',
//           }}
//         />
//         <input
//           type="email"
...
//           style={{
//             width: '100%', background: email && name ? accentColor : '#333',
//             color: email && name ? accentFg : '#666',
//             border: 'none', padding: '14px', fontSize: 14, fontWeight: 700,
...
//           }}
//         >
//           {loading ? <><Loader size={14} style={{ animation: 'spin 1s linear infinite' }} /> Processing...</> : <>Pay Now <ArrowRight size={16} /></>}
//         </button>
//         <button
...
//   // Step 3: paying — Paystack iframe is open (handled by SDK), show spinner
//   return (
...
//       <span style={{ fontSize: 13, color: '#888' }}>Opening payment...</span>
//       <style>{`@keyframes spin { to { transform: rotate(360deg); } }`}</style>
//     </div>
//   )
// }
